...
const downloadBill = (bill) => {
  ...
  const linesText = [
    ...
    `  DOB       : ${PATIENT.dob}   Blood: ${PATIENT.blood}`,
    `  Phone     : ${PATIENT.phone}`,
    "",
    "  BILL DETAILS",
    "──────────────────────────────────────────────────",
    ...
    ...bill.items.map((item, i) =>
      [
        ...
        `     Dosage     : ${item.dosage}`,
        `     Form       : ${item.form}`,
        `     Qty        : ${item.qty} units  @  Rs.${item.unitPrice.toFixed(2)} each`,
        `     Amount     : Rs.${(item.qty * item.unitPrice).toFixed(2)}`,
        `     For        : ${item.purpose}`,
        "",
      ].join("\n")
    ),
    ...
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          messages: [{
            role: "user",
            content: `You are a friendly hospital pharmacist. Explain this medicine to a patient in simple, clear language.

Medicine: ${item.name}
Dosage: ${item.dosage}
Form: ${item.form}
Prescribed for: ${item.purpose}

Respond ONLY with valid JSON (no markdown, no backticks):
{
  "purpose": "2-3 sentence plain English explanation of what this medicine does and why the patient is prescribed it",
  "dosageGuide": "Clear instruction on how and when to take this exact dose",
  "tips": ["practical tip 1", "practical tip 2", "practical tip 3"],
  "warning": "One key safety warning the patient must know"
}`
          }]
        })
      });
        ...
        <div style={{ padding: "1.4rem 1.8rem" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: ".8rem", marginBottom: "1.2rem" }}>
            ...
            <div style={{ padding: "12px 14px", borderRadius: 13, background: "rgba(255,255,255,.03)", border: "1px solid rgba(255,255,255,.07)" }}>
              ...
              {[
                ...
                { label: "Blood Group", value: PATIENT.blood, color: "#ff6b6b" },
          ...
          <div style={{ borderRadius: 14, border: `1px solid ${bill.color}22`, overflow: "hidden", marginBottom: "1rem" }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px 16px", background: `${bill.color}0c` }}>
              ...
              <span style={{ fontFamily: "'Syne',sans-serif", fontSize: "1.45rem", fontWeight: 800, color: bill.color }}>₹{(total + tax).toFixed(2)}</span>
            </div>
          </div>

          {/* ── DOWNLOAD ── */}
          <button onClick={() => downloadBill(bill)}
...
const BillRow = ({ bill, idx, onView }) => {
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "40px 1.4fr 1fr 1fr 1fr 80px 80px 100px", gap: 10, alignItems: "center", padding: "12px 18px", borderRadius: 13, background: "rgba(255,255,255,.025)", border: `1px solid rgba(255,255,255,.06)`, transition: "background .18s,border-color .18s", animation: "fadeUp .3s both", animationDelay: `${idx * .06}s` }}
      ...
      {/* total */}
      <div style={{ fontFamily: "'Syne',sans-serif", fontSize: ".88rem", fontWeight: 800, color: bill.color }}>₹{total.toFixed(2)}</div>
      {/* status */}
...
export default function MedicineBillsPage() {
  ...
  const totalSpent = BILLS.filter(b => b.status === "Paid").reduce((s, b) => s + billTotal(b), 0);
  const totalPending = BILLS.filter(b => b.status === "Pending").reduce((s, b) => s + billTotal(b), 0);
  ...
  return (
    <>
        ...
        <div className="main">
          ...
          <div className="topbar">
            <div className="topbar-left">
              ...
              <div className="page-sub">Purchase receipts for {PATIENT.name} · {PATIENT.id}</div>
            </div>
          ...
          <div className="hospital-banner">
            ...
            <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 8 }}>
              <div style={{ textAlign: "right" }}>
                ...
                <div style={{ fontFamily: "'Syne',sans-serif", fontSize: ".8rem", fontWeight: 700, color: "#00c8ff" }}>{PATIENT.uhid}</div>
              </div>
            </div>
          </div>
          ...
          <div className="content">
            ...
            {filtered.length > 0 ? (
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                {filtered.map((bill, i) => (
                  <BillRow key={bill.billNo} bill={bill} idx={i} onView={setViewBill} />
                ))}
              </div>
            ) : (
      ...
      {viewBill && <ViewModal bill={viewBill} onClose={() => setViewBill(null)} />}
    </>
  );
}
