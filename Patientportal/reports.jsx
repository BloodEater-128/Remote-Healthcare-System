...
const downloadReport = (r) => {
  ...
  const linesText = [
    ...
    `  UHID        : ${PATIENT.uhid}`,
    `  DOB / Age   : ${PATIENT.dob} / ${PATIENT.age}`,
    `  Gender      : ${PATIENT.gender}   Blood Group: ${PATIENT.blood}`,
    `  Ward        : ${PATIENT.ward}`,
    `  Phone       : ${PATIENT.phone}`,
    "",
    bar,
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: ".8rem", marginBottom: "1.2rem" }}>
            ...
            <div style={{ padding: "12px 14px", borderRadius: 13, background: "rgba(255,255,255,.03)", border: "1px solid rgba(255,255,255,.07)" }}>
              ...
              {[
                ...
                { l: "ID", v: PATIENT.id },
                { l: "UHID", v: PATIENT.uhid },
                { l: "Age/Sex", v: `${PATIENT.age} / ${PATIENT.gender}` },
                { l: "Blood", v: PATIENT.blood, c: "#ff6b6b" },
  ...
  return (
    <div style={{ background: "rgba(255,255,255,.03)", border: `1px solid ${r.color}18`, borderRadius: 18, padding: "1.15rem", position: "relative", overflow: "hidden", animation: "fadeUp .35s both", animationDelay: `${idx * .06}s`, transition: "border-color .2s,box-shadow .2s", cursor: "default" }}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: ".45rem", marginBottom: ".8rem" }}>
        {[
          { l: "Report ID", v: r.id },
          { l: "Date", v: r.reportedOn.split(" · ")[0] },
          { l: "Referred By", v: r.referredBy },
          { l: "Department", v: r.department.split(" – ")[0] },
          { l: "Sample", v: r.sampleType },
          { l: "Conducted At", v: r.conductedAt.split(",")[0] },
        ].map(({ l, v }) => (
          <div key={l}>
...
export default function ReportsPage() {
  ...
  const filtered = REPORTS.filter(r => {
    ...
    const matchSearch = !q || r.testName.toLowerCase().includes(q) || r.id.toLowerCase().includes(q) || r.referredBy.toLowerCase().includes(q) || r.department.toLowerCase().includes(q) || r.category.toLowerCase().includes(q);
  ...
  return (
    <>
        ...
        <div className="vp-main">
          ...
          <div className="vp-topbar">
            <div>
              ...
              <div className="vp-sub">Lab & diagnostic tests conducted at {HOSPITAL.name}</div>
            </div>
          ...
          <div className="hosp-bar">
            ...
            <div>
              <div style={{ fontFamily: "'Syne',sans-serif", fontSize: ".84rem", fontWeight: 800, color: "#fff" }}>{HOSPITAL.name} — {HOSPITAL.lab}</div>
              <div style={{ fontSize: ".6rem", color: "rgba(255,255,255,.28)", marginTop: 1 }}>{HOSPITAL.address} · {HOSPITAL.phone} · Lab Lic: {HOSPITAL.labLic}</div>
            </div>
            <div style={{ marginLeft: "auto", textAlign: "right" }}>
              <div style={{ fontSize: ".58rem", color: "rgba(255,255,255,.25)", textTransform: "uppercase", letterSpacing: ".08em" }}>Patient UHID</div>
              <div style={{ fontFamily: "'Syne',sans-serif", fontSize: ".78rem", fontWeight: 700, color: "#00c8ff" }}>{PATIENT.uhid}</div>
            </div>
          </div>
      ...
      {viewRpt && <ReportModal r={viewRpt} onClose={() => setViewRpt(null)} />}
    </>
  );
}
