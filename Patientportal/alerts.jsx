...
export default function AlertsPage() {
  ...
  const markAllRead = () => setLogItems(p => p.map(a => ({ ...a, unread: false })));
  const unreadItems = logItems.filter(a => a.unread);
  const updateAlert = (updated) => setAlerts(prev => prev.map(a => a.id === updated.id ? updated : a));
                    ...
                    {unreadItems.filter(a => a.severity === "High" || a.severity === "Medium").length > 0 && (
                      <div style={{ marginBottom: "1.2rem" }}>
                        ...
                        {unreadItems.filter(a => a.severity === "High" || a.severity === "Medium").map((a, i) => (
                          <div key={i} onClick={() => setLogItems(p => p.map(item => item.title === a.title ? { ...item, unread: false } : item))}
