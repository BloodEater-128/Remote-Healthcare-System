  ...
  return (
    <>
      ...
      <style>{`
        ...
        @keyframes glitch   { 0%{clip-path:inset(0 0 0 0)} 20%{clip-path:inset(20% 0 40% 0)} 40%{clip-path:inset(60% 0 10% 0)} 60%{clip-path:inset(10% 0 70% 0)} 80%{clip-path:inset(40% 0 30% 0)} 100%{clip-path:inset(0 0 0 0)} }
        
        *,*::before,*::after { box-sizing:border-box; margin:0; padding:0 }
        body { background:#040109; color:#fff; font-family:'DM Sans',sans-serif; overflow:hidden }
      `}</style>
      ...
      <div style={{
        ...
        {step === "confirm" && (
          <div style={{
            ...
            background: "linear-gradient(165deg,#1a0b1f,#08020a)",
            ...
            <div style={{ padding: "2.5rem 2rem" }}>
              <div style={{ textAlign: "center", marginBottom: "2rem" }}>
                ...
                <h1 style={{ fontFamily: "'Syne',sans-serif", fontSize: "1.6rem", fontWeight: 800, marginBottom: "0.5rem" }}>Permanent Deletion</h1>
                <p style={{ fontSize: "0.85rem", color: "rgba(255,255,255,.4)", lineHeight: 1.6 }}>
                  This action is irreversible. All medical records, patient history, and personal data will be purged from our secure servers.
                </p>
              </div>
              ...
              <div style={{
                ...
                <div style={{ width: 44, height: 44, borderRadius: 12, background: "linear-gradient(135deg,#9333ea,#c084fc)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "1.2rem" }}>👨‍⚕️</div>
                <div>
                  <div style={{ fontWeight: 700, fontSize: "0.9rem" }}>{DOCTOR.name}</div>
                  <div style={{ fontSize: "0.7rem", color: "rgba(255,255,255,.3)" }}>{DOCTOR.id} • {DOCTOR.department}</div>
                </div>
              </div>
        ...
        {step === "deleting" && (
          <div style={{ textAlign: "center", width: "100%", maxWidth: 400, position: "relative", zIndex: 2 }}>
            ...
            <h2 style={{ fontFamily: "'Syne',sans-serif", fontSize: "1.4rem", fontWeight: 800, color: "#fff", marginBottom: "0.5rem" }}>Purging Workspace</h2>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "8px", alignItems: "flex-start", maxWidth: 280, margin: "2rem auto 0" }}>
              {[
                { label: "Revoking medical credentials", t: 15 },
                { label: "Wiping patient charts", t: 40 },
                { label: "Clearing prescription history", t: 70 },
                { label: "Terminating cloud instances", t: 95 },
              ].map(s => (
                <div key={s.label} style={{ display: "flex", alignItems: "center", gap: "10px", opacity: progress >= s.t ? 1 : 0.3, transition: "opacity .3s" }}>
        ...
        {step === "done" && (
          <div style={{ textAlign: "center", animation: "scaleIn .5s cubic-bezier(.16,1,.3,1) both" }}>
            <div style={{
              ...
              display: "flex", alignItems: "center", justifyContent: "center",
              fontSize: "3rem", boxShadow: "0 0 50px rgba(220,38,38,.2)"
            }}>🛡️</div>
            <h1 style={{ fontFamily: "'Syne',sans-serif", fontSize: "1.8rem", fontWeight: 800, color: "#fff", marginBottom: "0.5rem" }}>Account Purged</h1>
            <p style={{ fontSize: "0.9rem", color: "rgba(255,255,255,.4)" }}>Successfully removed all data securely.</p>
          </div>
        )}
      </div>
    </>
